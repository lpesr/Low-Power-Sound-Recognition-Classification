...
def test_augmented_dataset(originalDataset, combinations, outputFile):
    ...
    numFolds = 5
    dataFolds = np.empty((numFolds, 0)).tolist()
    labelFolds = np.empty((numFolds, 0)).tolist()

    for combination in combinations:
        (dataFoldsBuffer, labelFoldsBuffer) = dp.prepare_input_data_with_folds(originalDataset + combination, ["glass_breaking", "siren", "hand_saw", "vacuum_cleaner", "crackling_fire"], 0.05, 1, 1, numFolds)#4000)#, "jackhammer", "siren", "dog_bark"], 1500) #"glass_breaking", "siren", "hand_saw", "vacuum_cleaner", "crackling_fire"
    ...
    outputFile.write(" ".join([name[1:] for name in combinations]) + "," + format(sum(map(float, results[0][:])) / len(results[j]), ".3f") + "," + format(sum(map(float, results[1][:])) / len(results[j]), ".3f") + "," + format(sum(map(float, results[2][:])) / len(results[j]), ".3f") + "\n")
    outputFile.flush()
...
run_evaluation(originalDataset, augmentedDatasets, "U:/GDP/ML Testing/Low-Power-Sound-Recognition-Classification/Src/ResultsFormatter/augmentationOutputESC-50.csv")